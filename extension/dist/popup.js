...
(() => {
  ...
  function parseAttendanceCsv(source) {
    const result = import_papaparse.default.parse(source.replace(/^\uFEFF/, ""), {
      header: true,
      skipEmptyLines: true
    });
    ...
    if (!result.data.length) throw new Error("CSV kh\xF4ng c\xF3 sinh vi\xEAn.");
    ...
    for (const row of result.data) {
      ...
      if (!row.roll_number || !["P", "A"].includes(row.status)) {
        throw new Error("CSV ch\u1EE9a MSSV ho\u1EB7c tr\u1EA1ng th\xE1i kh\xF4ng h\u1EE3p l\u1EC7.");
      }
      if (seen.has(row.roll_number)) throw new Error(`CSV tr\xF9ng MSSV ${row.roll_number}.`);
      seen.add(row.roll_number);
      if (row.class_code !== first.class_code || row.meeting_id !== first.meeting_id || row.meeting_date !== first.meeting_date || row.schedule_code !== first.schedule_code) {
        throw new Error("M\u1ED9t CSV ch\u1EC9 \u0111\u01B0\u1EE3c ch\u1EE9a m\u1ED9t l\u1EDBp v\xE0 m\u1ED9t bu\u1ED5i.");
      }
    }
    return result.data;
  }
  var normalizeId = (value) => value.trim().toUpperCase();

  // src/attendance_json.ts
  ...
  function parseAttendanceJson(source) {
    let raw;
    try {
      raw = JSON.parse(source.replace(/^\uFEFF/, ""));
    } catch {
      throw new Error("File JSON kh\xF4ng h\u1EE3p l\u1EC7.");
    }
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      throw new Error("H\xE3y ch\u1ECDn file JSON c\u1EE7a m\u1ED9t slot, kh\xF4ng ph\u1EA3i workspace.json.");
    }
    const session = raw;
    if (session.state !== "final") {
      throw new Error("Ch\u1EC9 nh\u1EADp file JSON c\u1EE7a slot \u0111\xE3 ch\u1ED1t (state: final).");
    }
    const meeting = session.meeting;
    if (!meeting || typeof meeting !== "object" || Array.isArray(meeting)) {
      throw new Error("JSON thi\u1EBFu th\xF4ng tin slot.");
    }
    const slot = meeting;
    ...
    if (Number.isNaN(Date.parse(startAt)) || Number.isNaN(Date.parse(endAt))) {
      throw new Error("Ng\xE0y gi\u1EDD slot trong JSON kh\xF4ng h\u1EE3p l\u1EC7.");
    }
    const students = session.attendance;
    if (!Array.isArray(students) || !students.length) {
      throw new Error("JSON kh\xF4ng c\xF3 sinh vi\xEAn.");
    }
    const base = {
    ...
    return students.map((value, index) => {
      if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new Error(`D\xF2ng sinh vi\xEAn ${index + 1} kh\xF4ng h\u1EE3p l\u1EC7.`);
      }
      const student = value;
      const roll = normalizeId(requiredString(student.rollNumber, `MSSV d\xF2ng ${index + 1}`));
      if (seen.has(roll)) throw new Error(`JSON tr\xF9ng MSSV ${roll}.`);
      seen.add(roll);
      const status2 = student.attendance?.status;
      if (status2 !== "P" && status2 !== "A") {
        throw new Error(`Sinh vi\xEAn ${roll} ch\u01B0a c\xF3 tr\u1EA1ng th\xE1i P/A.`);
      }
      return {
        ...base,
        roll_number: roll,
        ...
        full_name: requiredString(student.fullName, `h\u1ECD t\xEAn ${roll}`),
        status: status2,
  ...
  async function pickCsvFromDrive() {
    ...
    if (!response.ok) throw new Error("Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c CSV t\u1EEB Drive.");
    return response.text();
  }

  // src/popup.ts
  var file = document.querySelector("#attendanceFile");
  var drive = document.querySelector("#driveFile");
  var previewElement = document.querySelector("#preview");
  var confirmRow = document.querySelector("#confirmRow");
  var confirm = document.querySelector("#confirm");
  var autoSubmitRow = document.querySelector("#autoSubmitRow");
  var autoSubmit = document.querySelector("#autoSubmit");
  ...
  file.addEventListener("change", async () => {
    ...
    try {
      ...
      await prepare(selected.name.toLowerCase().endsWith(".json") ? parseAttendanceJson(source) : parseAttendanceCsv(source));
    } catch (error) {
      fail(error);
    }
  });
  drive.addEventListener("click", async () => {
    resetSelection();
    try {
      await prepare(parseAttendanceCsv(await pickCsvFromDrive()));
    } catch (error) {
      fail(error);
    }
  });
  ...
  submit.addEventListener("click", async () => {
    ...
    status.textContent = "\u0110ang \u0111i\u1EC1n v\xE0 ki\u1EC3m tra l\u1EA1i t\u1EEBng d\xF2ng\u2026";
    try {
      const result = await send({ type: "apply", rows, autoSubmit: autoSubmit.checked });
      if (result.error) throw new Error(result.error);
      status.textContent = !result.submitted ? "\u0110\xE3 \u0111i\u1EC1n P/A v\xE0 ki\u1EC3m tra l\u1EA1i. Ch\u01B0a b\u1EA5m Submit; GV h\xE3y ki\u1EC3m tra r\u1ED3i t\u1EF1 b\u1EA5m tr\xEAn trang." : result.success ? "Trang \u0111i\u1EC3m danh \u0111\xE3 b\xE1o l\u01B0u th\xE0nh c\xF4ng." : "\u0110\xE3 b\u1EA5m Submit nh\u01B0ng ch\u01B0a th\u1EA5y x\xE1c nh\u1EADn th\xE0nh c\xF4ng; h\xE3y ki\u1EC3m tra trang.";
      status.className = result.success || !result.submitted ? "ok" : "";
    } catch (error) {
      fail(error);
    }
  });
  async function prepare(parsedRows) {
    status.textContent = "";
    status.className = "";
    rows = parsedRows;
    ...
    if (result.error) throw new Error(result.error);
    const value = result;
    const blocking = value.missingOnPage.length || value.missingInCsv.length || value.duplicates.length || value.metadataWarnings.length;
    previewElement.hidden = false;
    previewElement.innerHTML = `
    <strong>${escape(rows[0].course_code)} \xB7 ${escape(rows[0].class_code)}</strong><br>
    Slot ${escape(rows[0].meeting_number)} \xB7 ${escape(rows[0].meeting_date)}<br>
    ${value.csvCount} sinh vi\xEAn \xB7 P: ${value.present} \xB7 A: ${value.absent}<br>
    ${blocking ? "<b>D\u1EEF li\u1EC7u ch\u01B0a kh\u1EDBp:</b> " + escape([
      ...value.missingOnPage.map((id) => `thi\u1EBFu tr\xEAn FAP ${id}`),
      ...value.missingInCsv.map((id) => `thi\u1EBFu trong file ${id}`),
      ...value.duplicates.map((id) => `tr\xF9ng ${id}`),
      ...value.metadataWarnings
    ].join("; ")) : "\u0110\u1ED1i chi\u1EBFu hai chi\u1EC1u th\xE0nh c\xF4ng."}`;
  ...
  async function send(message) {
    ...
    const allowed = url && (url.origin === "https://fap.fpt.edu.vn" || url.protocol === "http:" && ["localhost", "127.0.0.1"].includes(url.hostname));
    if (!tab.id || !allowed) {
      throw new Error("H\xE3y m\u1EDF trang \u0111i\u1EC3m danh FAP ho\u1EB7c trang demo localhost.");
    }
